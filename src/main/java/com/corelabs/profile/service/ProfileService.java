...
public class ProfileService {
    ...
    @Transactional
    public void saveProfile(ProfileVO profileVO, MultipartFile photo, List<MultipartFile> attaches) {
        String uuid = UUID.randomUUID().toString();
        if (photo != null && !photo.isEmpty()) {
            savePhoto(photo, uuid, profileVO);
        }
        ...
        if (attaches != null && !attaches.isEmpty()) {
            saveAttaches(attaches, uuid);
        }
    }

    // 증명사진 저장
    private void savePhoto(MultipartFile photo, String uuid, ProfileVO profileVO) {
    ...
    // 다중 파일 저장
    private void saveAttaches(List<MultipartFile> attaches, String uuid) {
